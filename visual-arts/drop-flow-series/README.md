# Drop Flow Series | 滴流系列

- [返回 Visual Arts](../README.md)
- [返回首页](../../README.md)
- [返回 Projects / Credits](../../README.md#projects--credits)

---

Drop Flow 系列围绕空间生成、沉浸式图像、身体表演与图像离开屏幕展开，是我近年关于空间叙事与感知迁移的重要作品线之一。它关心的不只是画面本身，而是图像如何与空间、身体、观看位置和光线关系一起工作。

## Public Graph / 对象关系

- `object_id`: `drop-flow`
- `view_type`: `personal_view`
- `authorship_type`: `team_work`
- `personal_view`: 当前页面，说明钱誉文在空间生成、现场图像与感知迁移中的个人实践线
- `team_view`: [VIRTURA-Collective / Drop Flow](https://github.com/ewanqian/VIRTURA-Collective/tree/main/works/drop-flow)
- `archive_view`: [VIRTURA-SpacePort / Drop Flow Archive](https://github.com/ewanqian/VIRTURA-SpacePort/tree/main/organization/works/drop-flow-series)
- `publication_view`: [从资料档案库到文化基因节点网络](https://github.com/ewanqian/VIRTURA-Newsroom/blob/main/essays/from-archive-library-to-cultural-gene-node-network.md)

同一件作品在不同仓库里不再重复从零解释：这里保留个人实践入口，团队语境看 `VIRTURA-Collective`，完整版本和资料索引看 `VIRTURA-SpacePort`，方法论解释看 `VIRTURA-Newsroom`。

## Why This Series Matters

这是我作品线中最能体现“空间生成”与“进入机制”的一条主线。它并不把图像当成平面内容，而是尝试让观众感受到自己正在进入一个被组织过的状态之中。

如果你想先理解我如何处理沉浸式空间、现场结构与图像外溢，建议从这个系列开始。

## Core Focus

- 空间生成 / Spatial Generation
- 沉浸式图像 / Immersive Imaging
- 身体与空间关系 / Body-Space Relation
- 图像离开屏幕 / Image Leaving the Screen
- 感知迁移 / Perceptual Migration

## Public Viewing

- [Bilibili｜滴流 Drop Flow | 上海 UFO Terminal 未知终端](https://www.bilibili.com/video/BV1PREczgEzC/?spm_id_from=333.1387.upload.video_card.click&vd_source=cb242067ab3ab2bc069ca22e37a86032)
- [Bilibili｜DROPFLOW 2 AUDIOVISUAL in Rooooooom719 排练现场](https://www.bilibili.com/video/BV1HZgBzWEre/?spm_id_from=333.1387.homepage.video_card.click&vd_source=cb242067ab3ab2bc069ca22e37a86032)

## Related Projects

- [杭州双年展开幕式《滴流》](../../projects/drop-flow-visual-2025.md)
- [Projects / 项目总表](../../projects/README.md)
- [Visual Arts / 视觉艺术系列总表](../README.md)

## Notes

这个系列目前在 portfolio 仓库中的文档仍以关键项目节点为主。建议第一次进入时先看公开视频，再看项目页，以理解它如何从现场实验、排练状态逐步发展为公开呈现的空间结构。

后续可继续补充：
- 系列版本演进
- 展演地点与设备差异
- 身体表演与画面之间的关系
- 与 TIMER / 扫描环境的交叉

## Related Links

- [返回 Visual Arts](../README.md)
- [返回首页](../../README.md)
