...
function setPreset(i,n){n=clamp(Math.round(n),0,4);if(n===preset[i])return;preset[i]=n;mods[i].y=YVAL[n];mods[i].vel=.18+n*.12;updateBus(i);paintZone(i);flash.push({i,born:performance.now(),kind:'preset'});status.textContent=`${MATERIALS[i].key} ${MATERIALS[i].name} · ${PRESETS[n]}`}
