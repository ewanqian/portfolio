...
> Mid-development log / Browser V0.2 → V0.3 / Generative audiovisual system
...
当前已实现的是音频输入驱动画面：麦克风、本地音频和 demo signal 提供 low / mid / high、onset 与 phrase gate，状态机决定何时允许内部变化与状态转移。

下一阶段的方向不是继续增加普通频谱映射，而是让视觉事件也能成为声音参数：横向位置对应声像，移动方向对应空间轨迹，速度与张力对应音色和能量，聚合与分散对应声场的收束和展开。这样，作品可以从“被声音驱动的画面”进一步发展为视觉与声音共享同一事件结构的乐器。

这一层目前属于研究方向。AI 对画面运动的分析、视觉到空间音频的推理和声音合成都尚未进入当前浏览器实现。
...
- 六个独立视觉状态及其内部变体；
- 有向加权 transition graph 与约 1.18 秒机械遮板转场；
- 麦克风设备选择、本地音频与 demo signal；
- `NEXT / HOLD / AUTO` 与默认隐藏的控制层；
- 会话内操作日志，不跨会话保存，也不修改权重。
